...
const DirectoryCityGrid = ({ name, description, directoryChildren, relativePrefixToRoot }: DirectoryGridProps) => {
    ...
    if (directoryChildren) {
        ...
        childrenDivs = sortedChildren.map((child: any) => (
            <div key={child.slug} className="border rounded-lg drop-shadow-md bg-gray-100 space-y-6 p-3 h-60">
                <h2>
                    <a className="font-bold text-2xl text-blue-700 hover:underline" href={relativePrefixToRoot + child.slug}>{child.name}</a>
                </h2>
